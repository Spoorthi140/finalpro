def generate_recommendations(products, causal_results=None, forecasts=None):
    ...
    for p in products:
        ...
        # Launch Promotion Recommendation
        if p.sales < 30 and p.marketing_spend < 100:
            recommendations.append({
                'product': p.product_name,
                'category': 'Demand Stimulation',
                'type': 'Growth',
                'priority': 'Medium',
                'message': f"Low Sales Volume Detected ({p.sales} units) and low marketing spend. Launching a high-impact promotional campaign is recommended to clear stagnant stock and stimulate customer demand.",
                'impact': "Clear Stagnant Stock",
